...
<style>
    ...
    .status-sim     { color: #EF9F27; font-weight: 600; }
</style>
...
def mini_chart(preds, color, fill_color):
...
with st.sidebar:
    ...
    if st.session_state.api_ok:
        st.markdown('<p class="status-online">● API online — live predictions</p>', unsafe_allow_html=True)
    else:
        st.markdown('<p class="status-sim">● Simulated mode</p>', unsafe_allow_html=True)
        st.caption("To enable live predictions, start Flask: `python app.py`")
...
def update_readings():
    ...
    for nid, profile in NURSE_PROFILES.items():
        ...
        if st.session_state.api_ok:
            ...
            # Simulated predictions based on real data profiles
            base = {"6D": 0.3, "BG": 1.1}.get(nid, 0.5)
            val  = max(0, min(2, base + random.gauss(0, 0.25)))
...
mode_label = "Live" if st.session_state.api_ok else "Simulated mode"
st.caption(f"Real-time physiological analysis · LightGBM vs GradientBoosting · {mode_label}")
...
m2.metric("High stress · LightGBM", high_lgbm)
m3.metric("High stress · GradBoost",high_gb)
...
for nid in NURSE_PROFILES:
    ...
    with lgbm_col:
        ...
        if latest["pred_lgbm"] is not None:
            ...
            if preds:
                st.plotly_chart(
                    mini_chart(preds, "#378ADD", "rgba(55,138,221,0.08)"),
    ...
    with gb_col:
        ...
        if latest["pred_gb"] is not None:
            ...
            if preds:
                st.plotly_chart(
                    mini_chart(preds, "#7F77DD", "rgba(127,119,221,0.08)"),
                    use_container_width=True, config={"displayModeBar": False}
                )
        else:
            st.caption("No data")
    ...
    with agree_col:
        if latest["pred_lgbm"] is not None and latest["pred_gb"] is not None:
            ...
            st.metric("Agree", "Yes" if match else "No")
            diff = abs(latest["pred_lgbm"] - latest["pred_gb"])
            if diff:
                st.caption(f"Delta {diff} label(s)")
...
hist_df = pd.DataFrame(st.session_state.history[selected_nurse]).dropna(subset=["hr"])
...
st.caption("Enter sensor values and compare both model predictions.")
    ...
    if st.button("Run prediction", use_container_width=True):
        if st.session_state.api_ok:
            try:
                ...
                if r_lgbm != r_gb:
                    st.warning(f"Models disagree — LightGBM: {r_lgbm}, GradBoost: {r_gb}")
                else:
                    st.success(f"Both models agree: {STRESS_LABELS[r_lgbm]}")
            except Exception as e:
                st.error(f"API error: {e}")
        else:
            st.info("Running in simulated mode. Start Flask to enable live predictions.")
